...
function handleRename(sessionId, argsStr) {
  try {
    ...
    if (two) {
      ...
      if (oldExists) {
        // Genuine two-arg rename of an existing thread
        const collides = maybeOld !== maybeNew &&
          db.prepare('SELECT name FROM saved_threads WHERE name = ?').get(maybeNew);
        if (collides) {
          db.close();
    ...
    const newName = stripOuterQuotes(args);
