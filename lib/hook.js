...
import { takeScreenshotAndPaste, pickFileAndAttach, fetchYoutubeTranscript } from './media.js';
...
// Media commands — both slash and literal forms
const SCREENSHOT_PATTERN = /^\/cloudctx(?:-screenshot|\s+screenshot)\s*$/im;
const UPLOAD_PATTERN = /^\/cloudctx(?:-upload|\s+upload)\s*$/im;
const YT_PATTERN = /^\/cloudctx(?:-yt|\s+yt)\s+(\S+)\s*$/im;
...
// Pending attachments queued by /cloudctx-screenshot and /cloudctx-upload.
// Drained on the next prompt and prepended to context so the user can compose
// a message AFTER capturing — natural flow: screenshot, then describe.
const PENDING_DIR = join(getDataDir(), '.pending-attachments');
...
export async function runHook() {
  ...
  if (SCREENSHOT_PATTERN.test(trimmed)) {
    handleScreenshot(sessionId);
    process.exit(0);
  }
  if (UPLOAD_PATTERN.test(trimmed)) {
    handleUpload(sessionId);
    process.exit(0);
  }
  const ytMatch = trimmed.match(YT_PATTERN);
  if (ytMatch) {
    await handleYoutube(ytMatch[1]);
    process.exit(0);
  }
  ...
  // Drain any queued attachments from /cloudctx-screenshot or /cloudctx-upload.
  // Inject paths as additional context so the agent reads them as part of THIS prompt.
  const pending = readPendingAttachments(sessionId);
  let attachmentBlock = '';
  if (pending.length) {
    clearPendingAttachments(sessionId);
    const list = pending.map(p => `  - ${p}`).join('\n');
    attachmentBlock = `[CloudCtx attached files] The user attached these via /cloudctx-screenshot or /cloudctx-upload — Read them to see what they're referring to:\n${list}\n\n`;
  }

  if (compactionDetected) {
    if (attachmentBlock) console.log(attachmentBlock);
    injectPostCompactionContext(sessionId);
  } else {
    console.log(attachmentBlock + `Reminder: ${REMINDER}`);
  }
...
function handleScreenshot(sessionId) {
  const r = takeScreenshotAndPaste();
  if (!r.ok) return reply(`📷 Screenshot ${r.error}.`);
  // Hook returns instantly so CC unblocks; the screenshot+paste runs in background.
  return reply(`📷 ${r.message}`);
}

function handleUpload(sessionId) {
  const r = pickFileAndAttach();
  if (!r.ok) return reply(`📁 File picker ${r.error}.`);
  if (r.kind === 'image') {
    // Image paste happens in detached background process; return immediately.
    return reply(`📁 ${r.message}`);
  }
  // Non-image file: queue path for next-prompt injection (legacy mechanism).
  appendPendingAttachment(sessionId, r.path);
  const queued = readPendingAttachments(sessionId).length;
  return reply(`📁 ${r.path} queued (${queued} pending). Type your prompt and submit — the agent will see the path.`);
}

function appendPendingAttachment(sessionId, path) {
  try {
    mkdirSync(PENDING_DIR, { recursive: true });
    appendFileSync(join(PENDING_DIR, sessionId), path + '\n');
  } catch {}
}

function readPendingAttachments(sessionId) {
  const file = join(PENDING_DIR, sessionId);
...
async function handleYoutube(url) {
  ...
  if (!r.ok) return reply(`🎬 YouTube transcript failed: ${r.error}`);
  // Inline the full transcript so the agent has it in the conversation.
  return reply(`🎬 YouTube transcript (${r.segmentCount.toLocaleString()} segments)\nURL: ${r.url}\n\n--- transcript ---\n${r.text}\n--- end transcript ---`);
}
