...
import { getReadonlyDb, getDb, dbExists, getDataDir } from './db.js';
import { processConversationFile } from './parser.js';

const CLAUDE_DIR = join(homedir(), '.claude');
const MARKER_DIR = join(getDataDir(), '.compaction-markers');

const REMINDER = `You have persistent memory via CloudCtx. Search it before guessing:
- Errors: cloudctx query "error AND keyword"
- Past work: cloudctx query "search terms"
- Docs: cloudctx sql "SELECT title, substr(content,1,300) FROM docs_fts WHERE docs_fts MATCH 'keyword' ORDER BY rank LIMIT 5"
- Recent: cloudctx sql "SELECT type, substr(content,1,200), timestamp FROM messages WHERE timestamp > datetime('now','-4 hours') ORDER BY timestamp DESC LIMIT 10"

CRITICAL: ALWAYS ASK the user before falling back to a different approach, making changes outside the requested scope, or taking actions the user did not ask for. Never silently switch methods, add unrequested features, or execute commands without confirmation.`;
